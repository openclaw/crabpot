...
function summarizeStartup(context) {
  ...
  try {
    ...
    for (const producer of ["parent", "worker", "powershell", "native"]) {
      const file = path.join(context.directory, `${producer}.jsonl`);
      try {
        if (statSync(file).size > diagnosticByteLimit) {
          current[producer] = { invalid: "byte-limit" };
          continue;
        }
      ...
      } catch { current[producer] = { records: 0, missingOrUnreadable: true }; }
    }
    const line = `crabpot-startup-305 ${JSON.stringify({
      attempt: context.attempt, id: context.id, snapshotUnixMs: Date.now(), current,
    })}\n`;
