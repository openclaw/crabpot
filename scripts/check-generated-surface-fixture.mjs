...
function runPluginInspector(pluginRoot, { runtime }) {
  const invocation = resolvePluginInspectorCliInvocation({ preferSource: runtime });
  const commandArgs = [
    ...invocation.args,
    "check",
    "--config",
  ...
  return {
    command: [invocation.command, ...commandArgs].join(" "),
    status: result.status,
    stdout: result.stdout,
    stderr: result.stderr,
    failures: result.status === 0 ? [] : [`plugin-inspector ${runtime ? "runtime" : "static"} failed with ${result.status}: ${result.stderr || result.stdout}`],
  };
}
