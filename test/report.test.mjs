...
test("compatibility report classifies current fixture seams", async () => {
  ...
  assert.equal(report.breakages.length, 2);
  ...
  assertHasFinding(report.warnings, "clawmetry", "sdk-export-missing");
  ...
  assertHasFinding(report.breakages, "hyperspell", "missing-expected-seam");
  ...
  assertHasDecision(report.decisions, "core-compat-adapter", "sdk-alias");
  ...
  assertHasIssue(report.issues, "P0", "missing-expected-seam");
  assertHasIssueClass(report.issues, "fixture-regression", "missing-expected-seam");
  ...
  assertHasProbe(report.contractProbes, "sdk.import.package-export-cold-import:clawmetry");
  assertHasProbe(report.contractProbes, "sdk.import.package-export-cold-import:clawmetry", "P1");
...
test("issue report preserves decision metadata for compat-layer work", async () => {
  ...
  assert.match(markdown, /🟠 P1 \*\*clawmetry\*\* `compat-gap` `core-compat-adapter`/);
  assert.match(markdown, /🟠 P1/);
  assert.match(markdown, /🟡 P2/);
  assert.match(
    markdown,
    /https:\/\/github\.com\/vivekchand\/clawmetry\/blob\/[0-9a-f]{40}\/clawhub-plugin\/src\/service\.ts#L2/,
  );
