...
| `aiwerk-mcp-bridge` | `plugins/aiwerk-mcp-bridge` | npm | medium | dynamic tool, MCP refresh, OAuth transport | MCP bridge fixture covering router/direct dynamic tool modes, MCP list refresh with `unregisterTool` fallback, remote auth transports, and bundled server recipes/skills. |
...
| `hapi-openclaw` | `plugins/hapi-openclaw` | npm | medium | gateway service, HTTP routes, service lifecycle | HAPI adapter fixture covering a native `/hapi` route surface, plugin-auth HTTP handlers, and transcript bridge/capture services backed by OpenClaw runtime events. |
